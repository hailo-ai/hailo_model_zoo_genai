...
class plain_guarded<T, M>::deleter
{
   public:
      ...
      // HAILO OLLAMA CHANGE: add access to lock member
      std::unique_lock<M>& as_lock();
...
// HAILO OLLAMA CHANGE: add access to lock member
template <typename T, typename M>
std::unique_lock<M>& plain_guarded<T, M>::deleter::as_lock()
{
  return m_lock;
}
...
// HAILO OLLAMA CHANGE: add access to lock member
template <typename T, typename M>
std::unique_lock<M>& plain_guarded<T, M>::const_deleter::as_lock()
{
  return m_lock;
}
